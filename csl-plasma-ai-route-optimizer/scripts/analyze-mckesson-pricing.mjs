...
function parseMileageBand(value) {
  const text = clean(value);
  const match = text.match(/^(\d+(?:\.\d+)?)\s*-\s*(\d+(?:\.\d+)?)/);
  if (!match) return null;
  return { label: text, min: Number(match[1]), max: Number(match[2]) };
}

function extractRateTableFormula(rawRows) {
  const bands = [];
  let dedicatedRate = 3.34;
  for (const row of rawRows) {
    row.forEach((cell, index) => {
      const text = clean(cell);
      if (/dedicated transportation rate per mile/i.test(text)) {
        const rowValues = row.map(numeric).filter((n) => n > 0 && n < 100);
        if (rowValues.length) dedicatedRate = rowValues.at(-1);
      }
      if (index === 0) {
        const band = parseMileageBand(text);
        if (band) {
          bands.push({
            ...band,
            ratesA: { 750: numeric(row[1]), 2200: numeric(row[2]), 5000: numeric(row[3]) },
            ratesB: { 750: numeric(row[6]), 2200: numeric(row[7]), 5000: numeric(row[8]) },
  ...
  return { bands, dedicatedRate };
}

function tierForWeight(weight) {
  if (!weight || weight <= 0) return '';
  if (weight <= 750) return '750';
  if (weight <= 2200) return '2200';
  return '5000';
}

function qualityCounts(rows) {
  const counts = { Strong: 0, Possible: 0, Weak: 0, 'No match': 0, Unknown: 0 };
  for (const row of rows) counts[row.quality || 'Unknown'] = (counts[row.quality || 'Unknown'] || 0) + 1;
  return counts;
}

function rateTableFormulaTest(invoiceRows, rawRateRows) {
  const rateTable = extractRateTableFormula(rawRateRows);
  const tested = [];
  const missing = { rateMiles: 0, weight: 0, linehaul: 0 };
  const noBand = [];
  const noTier = [];
  const candidateWins = {
    'Candidate A B/C/D table': 0,
    'Candidate B G/H/I table': 0,
    'Candidate C dedicated $3.34/mile': 0
  };
  for (const row of invoiceRows) {
    const route = clean(routeField(row)).toUpperCase() || '(missing)';
    const rateMiles = numeric(getField(row, [/rate miles/]));
    const weight = numeric(getField(row, [/^weight$/]));
    const actual = numeric(getField(row, [/linehaul amount/, /linehaul/]));
    if (!rateMiles) missing.rateMiles += 1;
    if (!weight) missing.weight += 1;
    if (!actual) missing.linehaul += 1;
    if (!rateMiles || !weight || !actual) continue;

    const band = rateTable.bands.find((candidate) => rateMiles >= candidate.min && rateMiles <= candidate.max);
    if (!band) {
      noBand.push({ route, row: row.sourceRowNumber, bol: getField(row, [/^bol$/i]), rateMiles, weight, actual });
      continue;
    }
    const tier = tierForWeight(weight);
    if (!tier) {
      noTier.push({ route, row: row.sourceRowNumber, bol: getField(row, [/^bol$/i]), rateMiles, weight, actual, band: band.label });
      continue;
    }
    const billedWeight = Math.max(weight, 750);
    const candidates = [
      { name: 'Candidate A B/C/D table', estimated: (band.ratesA[tier] || 0) * billedWeight / 100, tier, band: band.label, rate: band.ratesA[tier] || 0 },
      { name: 'Candidate B G/H/I table', estimated: (band.ratesB[tier] || 0) * billedWeight / 100, tier, band: band.label, rate: band.ratesB[tier] || 0 },
      { name: 'Candidate C dedicated $3.34/mile', estimated: rateMiles * rateTable.dedicatedRate, tier: 'per-mile', band: band.label, rate: rateTable.dedicatedRate }
    ].filter((candidate) => candidate.estimated > 0);
    if (!candidates.length) {
      noTier.push({ route, row: row.sourceRowNumber, bol: getField(row, [/^bol$/i]), rateMiles, weight, actual, band: band.label, tier });
      continue;
    }
    const best = candidates.map((candidate) => {
      const variance = candidate.estimated - actual;
      const variancePct = actual ? Math.abs(variance) / actual : null;
      return { ...candidate, variance, variancePct, quality: matchQuality(variancePct) };
    }).sort((a, b) => a.variancePct - b.variancePct)[0];
    candidateWins[best.name] += 1;
    tested.push({
      route,
      row: row.sourceRowNumber,
      bol: getField(row, [/^bol$/i]),
      stop: getField(row, [/stop location/, /center/]),
      rateMiles,
      weight,
      billedWeight,
      actual,
  ...
  const byRoute = [...groupBy(tested, (row) => row.route).entries()].map(([route, rows]) => ({
    route,
    rows: rows.length,
    avgAbsVariance: rows.reduce((sum, row) => sum + Math.abs(row.variance), 0) / rows.length,
    avgAbsVariancePct: rows.reduce((sum, row) => sum + (row.variancePct || 0), 0) / rows.length,
    strongestCandidate: Object.entries(rows.reduce((acc, row) => {
      acc[row.bestCandidate] = (acc[row.bestCandidate] || 0) + 1;
      return acc;
    }, {})).sort((a, b) => b[1] - a[1])[0]?.[0] || '(none)'
  })).sort((a, b) => a.avgAbsVariancePct - b.avgAbsVariancePct);
  return {
    totalRows: invoiceRows.length,
    testableRows: tested.length,
    missing,
    candidateWins,
    quality: qualityCounts(tested),
    byRoute,
    bestMatches: [...tested].sort((a, b) => a.variancePct - b.variancePct).slice(0, 25),
    worstMatches: [...tested].sort((a, b) => b.variancePct - a.variancePct).slice(0, 25),
    noBand: noBand.slice(0, 25),
    noTier: noTier.slice(0, 25),
    dedicatedRate: rateTable.dedicatedRate,
    bandCount: rateTable.bands.length
  };
}

function fuelSurchargeFormulaTest(invoiceRows) {
  const rows = invoiceRows.map((row) => {
    const linehaul = numeric(getField(row, [/linehaul amount/, /linehaul/]));
    const fuel = numeric(getField(row, [/fuel surcharge/]));
    if (!linehaul || !fuel) return null;
    const date = dateFromExcel(getField(row, [/pickup date/, /invoice date/]));
    return {
      route: clean(routeField(row)).toUpperCase() || '(missing)',
      month: monthKey(date) || 'unknown month',
      fuelPct: fuel / linehaul,
      linehaul,
      fuel
    };
  }).filter(Boolean);
  const summarize = (items, label) => {
    const values = items.map((row) => row.fuelPct).filter(Number.isFinite);
  ...
  const routeSummaryRows = [...groupBy(rows, (row) => row.route).entries()].map(([route, items]) => summarize(items, route)).sort((a, b) => b.rows - a.rows);
  const monthSummaryRows = [...groupBy(rows, (row) => row.month).entries()].map(([month, items]) => summarize(items, month)).sort((a, b) => a.label.localeCompare(b.label));
  const overall = rows.length ? summarize(rows, 'overall') : { label: 'overall', rows: 0, avg: 0, min: 0, max: 0, range: 0 };
  const consistency = overall.range <= 0.02 ? 'appears mostly consistent overall' : 'varies by route/month and needs McKesson validation';
  return { rows: rows.length, overall, routeSummaryRows, monthSummaryRows, consistency };
}
...
const rateFormula = rateTableFormulaTest(invoice.rows, rateTable.rawRows);
const fuelFormula = fuelSurchargeFormulaTest(invoice.rows);
...
printSection('Rate Table formula test against Invoice Detail FY26');
console.log(`Total invoice rows tested: ${rateFormula.totalRows}`);
console.log(`Rows with enough data to test: ${rateFormula.testableRows}`);
console.log(`Rows missing Rate Miles / Weight / Linehaul: rateMiles=${rateFormula.missing.rateMiles}, weight=${rateFormula.missing.weight}, linehaul=${rateFormula.missing.linehaul}`);
console.log(`Rate Table bands detected: ${rateFormula.bandCount}; dedicated transportation rate candidate: ${money(rateFormula.dedicatedRate)}/mile`);
console.log('Match rate by closest candidate (numeric reconciliation only; needs McKesson validation):');
for (const [candidate, count] of Object.entries(rateFormula.candidateWins)) {
  console.log(`  ${candidate}: ${count} row(s) (${pct(rateFormula.testableRows ? count / rateFormula.testableRows : 0)})`);
}
console.log(`Match quality counts: Strong=${rateFormula.quality.Strong}, Possible=${rateFormula.quality.Possible}, Weak=${rateFormula.quality.Weak}, No match=${rateFormula.quality['No match']}, Unknown=${rateFormula.quality.Unknown}`);
console.log(`Route-level average variance by route:\n${table(rateFormula.byRoute, [
  { label: 'Route', value: (r) => r.route },
  { label: 'Rows', value: (r) => r.rows },
  { label: 'Avg abs $ var', value: (r) => money(r.avgAbsVariance) },
  { label: 'Avg abs var %', value: (r) => pct(r.avgAbsVariancePct) },
  { label: 'Strongest candidate', value: (r) => r.strongestCandidate }
])}`);
const rateFormulaColumns = [
  { label: 'Route', value: (r) => r.route },
  { label: 'Row', value: (r) => r.row },
  { label: 'BOL', value: (r) => r.bol || '(blank)' },
  { label: 'Band', value: (r) => r.band },
  { label: 'Tier', value: (r) => r.tier },
  { label: 'Candidate', value: (r) => r.bestCandidate.replace('Candidate ', '') },
  { label: 'Actual', value: (r) => money(r.actual) },
  { label: 'Calc', value: (r) => money(r.estimated) },
  { label: 'Var', value: (r) => money(r.variance) },
  { label: 'Var %', value: (r) => pct(r.variancePct) },
  { label: 'Quality', value: (r) => r.quality }
];
console.log(`Top 25 best linehaul matches:\n${table(rateFormula.bestMatches, rateFormulaColumns, 25)}`);
console.log(`Top 25 worst linehaul mismatches:\n${table(rateFormula.worstMatches, rateFormulaColumns, 25)}`);
console.log(`Top 25 rows where no mileage band was found:\n${table(rateFormula.noBand, [
  { label: 'Route', value: (r) => r.route },
  { label: 'Row', value: (r) => r.row },
  { label: 'BOL', value: (r) => r.bol || '(blank)' },
  { label: 'Rate Miles', value: (r) => number(r.rateMiles) },
  { label: 'Weight', value: (r) => number(r.weight) },
  { label: 'Linehaul', value: (r) => money(r.actual) }
], 25)}`);
console.log(`Top 25 rows where weight tier could not be determined:\n${table(rateFormula.noTier, [
  { label: 'Route', value: (r) => r.route },
  { label: 'Row', value: (r) => r.row },
  { label: 'BOL', value: (r) => r.bol || '(blank)' },
  { label: 'Band', value: (r) => r.band || '(none)' },
  { label: 'Rate Miles', value: (r) => number(r.rateMiles) },
  { label: 'Weight', value: (r) => number(r.weight) },
  { label: 'Linehaul', value: (r) => money(r.actual) }
], 25)}`);
console.log('Interpretation: a strong row match appears to match numerically, but this does not confirm McKesson’s formula. Non-matches do not reconcile and need McKesson validation.');

printSection('Fuel surcharge formula test');
console.log(`Rows with linehaul and fuel surcharge: ${fuelFormula.rows}`);
console.log(`Overall fuel %: avg=${pct(fuelFormula.overall.avg)}, min=${pct(fuelFormula.overall.min)}, max=${pct(fuelFormula.overall.max)}, range=${pct(fuelFormula.overall.range)}; ${fuelFormula.consistency}`);
console.log(`Fuel % by route:\n${table(fuelFormula.routeSummaryRows, [
  { label: 'Route', value: (r) => r.label },
  { label: 'Rows', value: (r) => r.rows },
  { label: 'Avg fuel %', value: (r) => pct(r.avg) },
  { label: 'Min', value: (r) => pct(r.min) },
  { label: 'Max', value: (r) => pct(r.max) },
  { label: 'Range', value: (r) => pct(r.range) }
])}`);
console.log(`Fuel % by invoice month:\n${table(fuelFormula.monthSummaryRows, [
...
console.log('Interpretation: fuel surcharge percentages appear to match only if the route/month ranges are tight; otherwise fuel logic varies and needs McKesson validation.');
