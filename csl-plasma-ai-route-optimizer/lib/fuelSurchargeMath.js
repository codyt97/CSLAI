...
export function getFuelSurchargeAudit({ dieselAverage } = {}) {
  ...
  const rows = invoiceAudit.rows.map((row) => {
    ...
    } else if (actualFuelSurchargePercent !== null && Math.abs(actualFuelSurchargePercent - expectedPercent) > 1) {
  ...
  const status = expectedPercent === null ? STATUS.NEEDS_DIESEL : abnormalFuelPercentRows || Math.abs(variancePercent || 0) > 0.5 ? STATUS.REVIEW : STATUS.OK;
